resource "null_resource" "install_docker_brain" {    
    ...
    provisioner "remote-exec" {
        inline = [
            ...
            "echo ${var.ssh_password} | ${rancher2_cluster.cluster.cluster_registration_token.node_command} --etcd --controlplane",
        ]
...
resource "null_resource" "install_docker_storage" {    
    ...
    provisioner "remote-exec" {
        inline = [
            "echo ${var.ssh_password} | sudo -S curl ${var.bastion_host}/install_docker.sh | sudo -S BASTION_HOST=${var.bastion_host} DOCKER_VERSION=${var.docker_version} bash - ",
            "echo ${var.ssh_password} | sudo -S usermod -aG docker ${var.ssh_user}",
            "echo ${var.ssh_password} | ${rancher2_cluster.cluster.cluster_registration_token.node_command} --worker --label app=storage --taints storage=services:NoSchedule --internal-address ${each.key} --address ${var.cluster_nodes.storage[each.key]}",
        ]
...
resource "null_resource" "install_docker_worker" {    
    ...
    provisioner "remote-exec" {
        inline = [
            "echo ${var.ssh_password} | sudo -S curl ${var.bastion_host}/install_docker.sh | sudo -S BASTION_HOST=${var.bastion_host} DOCKER_VERSION=${var.docker_version} bash - ",
            "echo ${var.ssh_password} | sudo -S usermod -aG docker ${var.ssh_user}",
            "echo ${var.ssh_password} | ${rancher2_cluster.cluster.cluster_registration_token.node_command} --worker",
        ]
